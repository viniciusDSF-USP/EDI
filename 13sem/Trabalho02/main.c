...
int main(){
    ...
    AVL *Alunos = CriarAVL();
    AVL *Filmes = CriarAVL();
    char *string = (char *) malloc(sizeof(STRLIM));
    elem x;
    No **p;
    	...
    	switch(choice){
    		...
    		case 3: // Adicionar filme predileto
    			...
    			if (p != NULL){
					...
					if (inserir(&Filmes->raiz, &x)){
						p = buscar(&Filmes->raiz, &x);
						
						if (inserir_filme_fav(&(*p)->info.aluno.filmes_fav, p))
    		...
    		case 4: // Remover filme predileto
    			...
    			if (p != NULL){
					...
					if (*p != NULL){
						if (remover_filme_fav(&(*p)->info.aluno.filmes_fav, p)){
							if ((*p)->info.filme.frequencia == 0){
