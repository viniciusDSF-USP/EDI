...
void ler_recursivo(AVL *alunos, AVL *filmes, FILE* file) { // Arrumar
    elem* e;
    e->tipo = Aluno;
...
int ler_de_arquivo(No **a, No **f, char *nome_arquivo){ // Arrumar
    FILE *file;
    ...
    int nroUsp = atoi(strtok(NULL, ";"));
    char* token;
    while ((token = strtok(NULL, ";")) != NULL) {
        inserir();
    }
    
    ler_recursivo(a, file);
}